...
### Modules (Go packages — config, agent contract, claude/opencode drivers, store schema + kanban + card CRUD + trace run lifecycle + session tmux wrapper + SessionManager + BoardService + CLI router + card commands + session commands implemented, TUI shell + session markers (T16–T17), TUI forms (T18), TUI card detail (T19), TUI search + board/workspace switch/help (T20) — the TUI is fully implemented per DESIGN-002 §4.2)
