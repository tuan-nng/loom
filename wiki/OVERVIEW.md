...
### Modules (Go packages — config, agent contract, claude/opencode drivers, store schema + kanban + card CRUD + trace run lifecycle + session tmux wrapper + SessionManager + BoardService + CLI router + card commands + session commands implemented, TUI shell + session markers (T16–T17), TUI forms (T18), TUI card detail/search planned per DESIGN-002 §4.2 (T19–T20))
