...
- **Board shell (T16):** 5-column default (Backlog/To Do/In Progress/Review/Done) via `lipgloss.JoinHorizontal`; each column a `bubbles/list` with the custom card delegate. The lists keep only cursor + paging keys — the board owns h/l, `/`, q, `?` (each list's own bindings are disabled in `columnKeyMap`). Since the restyle each column is a **filled panel**: a titled top border (`columnTitleBar` puts the name + card count into the rule), a framed body (`columnView`/`boardView` with a `colGap` divider), framed by a `headerBar` (brand chip + `workspace › board` breadcrumb + search/card-count) and the `statusBar`.
...
- **Session markers + badges (T17, restyled):** each card is a two-line cell — line 1 has a priority rule (`▎` coloured by `store` priority), the §14 agent badge (`cl`/`oc` from `card.AgentOrDefault(cfg default)`), a truncated bold title, and a `●` running / `◉` attached marker pinned right; line 2 renders the description (truncated to the column, `truncateText`), or a plain fill so every row stays uniformly two cells tall (`delegate.Height() = 2`). Markers refresh every 2s without a keystroke.
- **Enter → open+attach (T17):** ensures the card's session via `svc.OpenCard(ctx, id, detach=true)` (the ensure's probe window never blocks Update), then hands the terminal to `session.AttachCommandFor(server, loom-<id>)` through `tea.ExecProcess` so BubbleTea owns the TTY and restores the board on detach. The handoff is the enclosing-aware attach: inside the user's tmux it links the card's window in as a plain tab (returning almost immediately); standalone it attaches to the dedicated `-L` server.
...
- `internal/tui/board.go` — `cardItem`/`cardDelegate` render (restyle: two-line rows with priority rule + description, `truncateText`, `cardDelegate{focused}`), `buildLists`, cursor-preserving `refreshMarkers`, `relayout`/`layout`, the restyled `boardView`/`columnView`/`columnTitleBar`/`headerBar`/`statusBar` (+ `columnWidth`, `sessionSummary`, `hints` derived from `defaultKeyMap` via `firstKey`), `agentBadge`; T20: `visibleCards` feeds `buildLists`/`listCount` so the filter narrows counts too
- `internal/tui/forms.go` — the T18 overlay: `form`/`field` (text + cycle), `formKeyMap` + per-input keymap (tab/up/down freed for the overlay), `openNewCardForm`/`openEditCardForm`/`openNewColumnForm`/`openMoveCardForm` (the new-card form now includes `nfDescription`), `cardInput`/`cardUpdate`/`columnInput` marshalling (nil vs `&""` vs value), `view` (`lipgloss.Place` centered box, title rule + two-line hint since the restyle), the four submit msgs + `after*` handlers, `formUpdate` (the keyPress guard), and the styles; T20: the `formSearch`/`formBoardSwitch`/`formWorkspaceSwitch` kinds and their submit routing
...
- `internal/tui/theme.go` — the restyle's palette layer: `palette` (dark/light), `darkPalette`/`lightPalette`, `applyTheme(isDark)` rebinding every style (column chrome, row styles, markers, bars, overlays, splash/error), `priorityColor`, `truncateText`, `barLine`
...
- `internal/tui/app_test.go` — `fakeService` seam (incl. the five T18 write methods, the T19 `GetCodebase`/`RunsForCard` mirror, and the T20 `boards`/`workspaces`/`boardsErr`/`workspacesErr`/`showBoardID`/`switchWsID` fields + `ListBoards`/`ShowBoard`/`ListWorkspaces`/`SwitchWorkspace`) + BubbleTea-driven tests (navigation, layout, quit confirm/force, badge/markers, completion toast, open/kill, attach handoff argv, poll re-arm; restyle: `TestCardRowShowsDescription` — two-line rows render the description and title-only cards keep a full-height row — and `TestCardRowTruncatesLongDescription` pinning the truncation math)
- `internal/tui/forms_test.go` — T18 acceptance: overlay opening, new-card submit + marshalling + refocus (now exercising the description field), agent nil-vs-pinned, validation, cancel, tab/shift+tab/left/right navigation (updated for the extra description field), edit seeding + three-way update marshalling, edit agent reset to NULL, new-column, move picker routing + follow-the-card refocus, key capture (q types, j swallowed), poll-under-form, move-to-done routing, empty-column degradation
- `internal/tui/search_test.go` — T20 acceptance: `/` narrows the board (matching cards stay, the column title-bar count narrows), title/description + case-insensitive matching, esc restores the prior filter, empty submit clears, the filter survives re-fetches
...
- Enter = ensure + attach through [BoardService.OpenCard](../modules/board.md) + the tmux attach handoff (`session.AttachCommandFor`; a linked tab inside the user's own tmux, `attach-session` standalone); `K` kill/finalizes; the 2s poll drives markers and completion toasts — the [card open → completion](../flows/card-open-complete.md) flow.
