...
- **Closed validation sets exported (T18)** — `store.ValidPriorities` (`low|medium|high`, display order) and `store.ValidStages` (derived from `DefaultColumns`, so it can never drift from the seeded template) are the single source of the two CHECK sets; the CLI validates against them (`slices.Contains`) with byte-identical messages, and the TUI's priority/stage cycle fields seed from them.
