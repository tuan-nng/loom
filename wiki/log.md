...
- source_commit: 34e55b3 (was 5b99765)
- Coverage: T20 TUI extras landed as real Go source since the last stamp — `internal/tui/app.go` (Service seam widened with `ListWorkspaces`/`ListBoards`/`ShowBoard`/`SwitchWorkspace`, auto-satisfied by the embedded `*board.Service`; model fields `search`/`searching`/`searchQuery`/`help`; msgs `boardsMsg`/`workspacesMsg`/`boardSwitchedMsg`/`workspaceSwitchedMsg`; keyPress routes `Search`→`startSearch`, `Board`/`Workspace`→`boardsCmd`/`workspacesCmd`, `Help`→`m.help = true`; guard order quit-confirm → form → detail → search → help; the old `noteSearch`/`noteBoard`/`noteWorkspace`/`noteHelp` stub constants removed), `internal/tui/board.go` (`filteredCards`/`filterCards` — title OR description case-insensitive substring, the same client-side semantics as `loom card list --search`; `buildLists`/`listCount` narrowed by the filter, `/"query"` status-bar indicator, `searchOverlay`/`helpOverlay` centered via `lipgloss.Place`, `searchInputWidth = 36`, `helpKeylines` canonical §3.5 table), and `internal/tui/forms.go` (the `formSwitchBoard`/`formSwitchWorkspace` kinds + `sbBoard`/`swWorkspace` field indexes, `openBoardSwitchForm`/`openWorkspaceSwitchForm` single cycle fields seeded at the current selection via `indexOf`, submit routing to `ShowBoard`/`SwitchWorkspace`, `applyBoards`/`applyWorkspaces`/`afterBoardSwitched`/`afterWorkspaceSwitched` handlers). Search: enter commits the filter, esc cancels keeping the prior filter, empty query clears; `m.cards` keeps the full snapshot. Switch: picker fetches run off the critical path (`boardsMsg`/`workspacesMsg`), submit persists the selection to `ui_state` and re-fetches with a toast. Help: dismiss-only, any key closes. `fakeService` gained `workspaces`/`boards`/`showBoardID`/`showBoardErr`/`switchWsID`/`switchWsErr` + the four methods; the six new tests `TestSearchFiltersBoard`/`TestSearchMatchesDescription`/`TestSearchEmptyCommitClearsFilter`/`TestHelpOverlayRendersAndCloses`/`TestBoardSwitchPickerAndSubmit`/`TestWorkspaceSwitchPickerAndSubmit` replace `TestStubKeysSetNotice` (`tea.KeyBackspace` is 0x7F, not `\b`). Full `go build`/`go vet`/`go test ./...` green. The whole T16–T20 TUI surface is now real — no stubbed keys remain. Updated the tui module page (T20 responsibilities, widened Service, key-file notes, fakeService + test list), the implementation-state framing in Overview + Architecture Overview (backlog fully implemented), and re-stamped source_commit.
