...
- Profile: internal/standard
- source_commit: 6f1df31 (was b22a5a4)
- Coverage: post-T22 polish landed since the last stamp — the **TUI visual restyle** (commits 16e5737 + 6f1df31): a new `internal/tui/theme.go` holds a dark/light `palette` with `applyTheme` rebinding every style, swapped on `tea.RequestBackgroundColor` (probed on the first resize via the `themeQueried` gate); columns became filled panels with the name + card count set into the top border (`columnTitleBar`), framed by a `headerBar` (loom brand chip, workspace › board breadcrumb, pinned search chip + card count) and a `statusBar` whose key hints derive from `defaultKeyMap()`; card rows are two-line cells — priority rule, agent badge, truncated title, ●/◉ marker pinned right, description (truncated) on line two, uniform 2-cell height; the `n` new-card form gained a description field (`nfDescription`); detail/help/forms overlays got title-rule + box-width chrome. The **enclosing-tmux attach handoff** (commits ee737ab + 83c4e44): when loom runs inside the user's tmux (`$TMUX`), sessions are managed on that enclosing server (`-S`) and `AttachCommand`/`AttachCommandFor` link the card's window in as a plain tab (`link-window`/`select-window`; `NewSession` names the first window `-n`), standalone it attaches on `-L`; `configureServer` applies loom-owned settings (prefix C-a, `status off`, `detach-on-destroy off`) to the `-L` server only, re-applied on reuse/attach; `KillSession` kills the linked window first on an enclosing server; `EnclosingSocket`/`enclosingSession`/`linkedWindow` resolve the target from `$TMUX`/`$TMUX_PANE`. **opencode default interface flipped to `full`** (commit 5e39207) in config. **justfile** added (commit ee737ab: build/install/test/vet/fmt/fmt-check/run/clean with a git-version ldflags). README gained the "Using the loom UI" section (commit 57e6ce0). Refreshed the session module page + Session Model + session concept (enclosing model, `configureServer`, `AttachCommand`/`AttachCommandFor`, linked-window kill; new tests `TestServerConfigured`, `TestEnclosingSocket`, `TestAttachCommandEnclosingLinksAsTab`, `TestKillSessionEnclosingKillsLinkedWindow`, `TestConfigureServerSkippedWhenEnclosing`), the tui module page + Architecture Overview (restyle, theme.go, two-line rows, description field, attach handoff), the config module page + opencode-launch-semantics guide (`full` default), the attach-detach + card-open-complete flows (link-as-tab handoff), the cli module page (`AttachCommandFor`, prefix forwarding), the session-command guide (`-n` first window), and re-stamped source_commit.
- Pages: [Overview](./OVERVIEW.md), [Architecture Overview](./architecture/overview.md), [Session Model](./architecture/session-model.md), [session](./modules/session.md), [tui](./modules/tui.md), [config](./modules/config.md), [cli](./modules/cli.md), [Attach/detach](./flows/attach-detach.md), [Card open → completion](./flows/card-open-complete.md), [session](./concepts/session.md), [opencode launch semantics](./guides/opencode-launch-semantics.md), [Session command construction](./guides/session-command.md)
