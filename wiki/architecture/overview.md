...
- **SQLite store** — `modernc.org/sqlite` (pure Go, no CGO), WAL mode, `foreign_keys=ON`, goose migrations. T4 landed the connection pragmas + migrations (`Open`); the CRUD layer is planned (T5–T7). See [Data Model](./data-model.md).
