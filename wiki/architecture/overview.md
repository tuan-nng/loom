...
- **SQLite store** — `modernc.org/sqlite` (pure Go, no CGO), WAL mode, `foreign_keys=ON`, goose migrations. T4 landed the connection pragmas + migrations (`Open`); T5 landed the kanban CRUD (workspaces/boards/columns/codebases, `ui_state`, `loom init`). Card reorder + trace lifecycle planned (T6–T7). See [Data Model](./data-model.md).
